...
import { ZARIT_ITEMS, ZARIT_OPTIONS, interpretZarit, deltaZarit } from "@/lib/clinical/zarit";

export const Route = createFileRoute("/_app/paciente/$id/escalas/zarit")({
  component: ZaritPage,
});
...
function ZaritPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data: p } = await supabase.from("patients").select("zarit_basal").eq("id", id).maybeSingle();
      setBasal((p?.zarit_basal as number | null) ?? null);
  ...
  const done = Object.keys(respuestas).length === ZARIT_ITEMS.length;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container-app pb-32 space-y-6">
        <p className="text-muted-foreground">Responda pensando en cómo se ha sentido el último mes cuidando a su familiar.</p>
        ...
        {done && (
          <div className="rounded-2xl bg-secondary p-4">
            <p className="font-display text-lg">Puntaje: <b>{total}/28</b></p>
            <p className="text-muted-foreground">{interpretZarit(total)}</p>
            <p className="text-sm text-muted-foreground mt-1">{deltaZarit(total, basal)}</p>
          </div>
        )}
        ...
        {historial.length > 0 && (
          <section className="rounded-3xl bg-card border border-border/60 p-5">
            ...
            {basal != null && (
              <p className="text-sm text-muted-foreground mb-2">Basal: <b>{basal}/28</b></p>
            )}
            <ul className="space-y-1.5">
              {historial.map((e) => (
                <li key={e.id} className="flex justify-between text-sm border-b border-border/40 last:border-0 py-1.5">
                  ...
                  <span className="font-medium">{e.puntaje}/28 · {interpretZarit(e.puntaje)}</span>
                </li>
              ))}
            </ul>
          </section>
